...
where the margin of error $ME = \text{critical value} \times SE$. The critical value for a $100(1-\alpha$)% CI can be computed by `qnorm(p = 1-alpha/2)` assuming the sampling distribution follows a normal distribution. For example, a 90% CI can be calculated as 100(1-0.1)% 
```{r}
alpha = 0.1
qnorm(p = 1-alpha/2)
```

lastly the standard error can be approximated using $SE = \frac{\sigma}{\sqrt{n}} \approx \frac{s}{\sqrt{n}}$, where $s$ is the standard deviation of an empirical sample

Putting this together, a 95% CI is 
$$
...
For our Starbucks calories example, A sample mean from a representative sample is $\overline{x} =$ `r round(xbar_calories,3)` and standard deviation $s=$ `r round(sd_calories, 3)` then the 95% CI is 
